...
public abstract partial class NonogramDisplay : Container
{
	...
	public required IConfig Config
	{
		get; set
		{
			Background.UniformPadding(IConfig.Margin);
			Hints.PlaceLabels(value);
	...
	protected static void UpdateDisplay(Button button, Nonogram.PenMode mode)
	{
		button.Text = mode switch
		{
			...
			Nonogram.PenMode.Fill when button.Text is EmptyText => FillText,
