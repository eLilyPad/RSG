	...
	public sealed record PuzzleData : NonogramDisplay.IData
	{
		...
		public Dictionary<NonogramDisplay.HintPosition, List<int>> Hints()
		{
			...
			foreach ((Vector2I position, bool check) in _checkedTiles)
			{
				...
				if (check)
				{
					if (column.Count is not 0 && column[^1] >= 1)
					{
						++column[^1];
					}
					else if (column.Count is not 0 && column[^1] == 0)
					{
						column.Add(1);
					}
					else
					...
					if (row.Count is not 0 && row[^1] >= 1)
					{
						++row[^1];
					}
					else if (row.Count is not 0 && row[^1] == 0)
					{
						row.Add(1);
					}
					else
				...
				{
					//if (column.Count >= 1 && column[^1] >= 1) ++column[^1]; else column.Add(1);
					//if (row.Count >= 1 && row[^1] >= 1) ++row[^1]; else row.Add(1);
					//column.Add(0);
					//row.Add(0);
				}
			}
