...
using NonogramHint = UI.NonogramDisplay;
using UI;
...
public partial class Nonogram : Node
{
	...
	public PuzzleData CurrentData
	{
		get; private set
		{
			PainterDisplay.Hints.WriteToLabels(value);
			GameDisplay.Hints.WriteToLabels(value);
		}
	} = new();
	...
	public PaintContainer PainterDisplay => field ??= new(nonogram: this);
	...
	public override void _Ready()
	{
		...
		PainterDisplay.Hints.WriteToLabels(CurrentData);
	}
	...
	public sealed record PuzzleData : NonogramDisplay.IData
	{
		...
		public Dictionary<NonogramHint.Position, List<int>> Hints()
		{
			Dictionary<NonogramHint.Position, List<int>> hints = [];
			foreach ((Vector2I position, bool check) in _checkedTiles)
			{
				NonogramHint.Position rowPos = new(NonogramHint.Side.Row, position.X),
					columnPos = new(NonogramHint.Side.Column, position.Y);
		...
		Dictionary<NonogramHint.Position, List<int>> NonogramHint.IData.Hints()
		{
			throw new NotImplementedException();
		}
	}

	public readonly record struct DisplayConfig(int Length = 5, int Scale = 40, int Margin = 150) : NonogramHint.IConfig;
	public sealed partial class PaintContainer(Nonogram nonogram) : NonogramHint
	{
		...
		public override void OnTilePressed(Button button, Vector2I position)
		{
			...
			Hints.WriteToLabels(nonogram.CurrentData);
		}
	}
