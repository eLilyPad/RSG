...
public partial class Nonogram : Node, IHavePenMode
{
	...
	public sealed record PuzzleData
	{
		...
		public Dictionary<NonogramDisplay.TileHints.Position, List<int>> Hints()
		{
			...
			foreach ((Vector2I position, bool check) in _checkedTiles)
			{
				if (check)
				{
					hints[new(NonogramDisplay.TileHints.Side.Column, position.X)].Add(1);
					hints[new(NonogramDisplay.TileHints.Side.Row, position.Y)].Add(1);
				}
				else
				{
					//hints[0] = [.. hints[0].Append(1)];
					//hints[1] = [.. hints[1].Append(1)];
				}
	...
	{
		public override void OnTilePressed(Button button)
		{
		...
		private void DisplayTilesOnHints()
		{
			foreach (Vector2I position in Tiles.ButtonPositions)
			{

			}
			//foreach (var type in GridHints)
		}
	}
