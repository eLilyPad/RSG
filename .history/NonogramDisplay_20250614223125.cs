...
public abstract partial class NonogramDisplay : Container
{
	...
	public TilesContainer Tiles => field ??= new() { Config = Config };
	...
	public sealed partial class TilesContainer : GridContainer
	{
		public required IConfig Config
		{
			set
			{
				...
				CustomMinimumSize = value.TilesSize;
				Columns = value.Length;
			}
		}
		private Dictionary<Vector2I, Button> Buttons { get; } = [];
		internal TilesContainer()
		{
			Name = "Tiles";
			FreeAll();
	...
	public sealed record TileHints
	{
		...
		{
			public HintLabel(
				Dictionary<TileHints.Position, RichTextLabel> labels,
				TileHints.Position position
			)
			{
				Text = "0";
				FitContent = true;
		...
		public readonly record struct Positions(Position Row, Position Columns)
		{
			public static explicit operator Positions(int index)
			{
				return new(new(Side.Row, index), new(Side.Column, index));
			}
		}
