...
public partial class Nonogram : Node
{
	...
	public sealed record PuzzleData : IData
	{
		...
		public Dictionary<HintPosition, List<int>> Hints()
		{
			...
			IEnumerable<(List<int> row, List<int> columns, bool check)> a = _checkedTiles.Select(pair =>
			{
				(HintPosition rowPos, HintPosition columnPos) = HintPosition.ToPosition(pair.Key);
				...
				return (row, column, pair.Value);
			});

			foreach ((List<int> row, List<int> columns, bool check) in _checkedTiles.Select(pair =>
			{
				(HintPosition rowPos, HintPosition columnPos) = HintPosition.ToPosition(pair.Key);
				if (!hints.TryGetValue(rowPos, out var row)) { hints[rowPos] = row = []; }
				if (!hints.TryGetValue(columnPos, out var column)) { hints[columnPos] = column = []; }
			...
			return hints.ToDictionary(
				keySelector: pair => pair.Key,
				elementSelector: pair => hints[pair.Key] = [.. pair.Value.Where(i => i is not 0)]
			);
		}
	}
