...
public partial class Nonogram : Node, IHavePenMode
{
	...
	public override void _Ready()
	{
		...
		PainterDisplay.Hints.WriteToLabels(hints: CurrentData.Hints());
	}
	...
	public sealed record PuzzleData
	{
		...
		public Dictionary<NonogramDisplay.TileHints.Position, List<int>> Hints()
		{
			...
			{
				NonogramDisplay.TileHints.Position rowPos = new(NonogramDisplay.TileHints.Side.Row, position.X);
				NonogramDisplay.TileHints.Position columnPos = new(NonogramDisplay.TileHints.Side.Column, position.Y);

				if (!hints.TryGetValue(rowPos, out List<int>? row)) { hints[rowPos] = row = []; }
				if (!hints.TryGetValue(columnPos, out List<int>? column))
				{
					column = [];
					hints[columnPos] = column;
				}
