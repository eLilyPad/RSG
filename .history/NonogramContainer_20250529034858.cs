		...
		public NonogramGrid(T data)
		{
			...
			for (int x = 0; x < GridSize; x++)
			{
				for (int y = 0; y < GridSize; y++)
				{
					...
					NonogramButton button = new(data, x, y);
					Buttons[position] = button;
					yield return button;
				}
			}

			this.Add([.. GridButtons(data)]);
		}
		private IEnumerable<Button> GridButtons(T data)
		{
			for (int x = 0; x < GridSize; x++)
			{
				for (int y = 0; y < GridSize; y++)
				{
					yield return Buttons[new(x, y)] = new NonogramButton(data, x, y);
				}
			}
		}
	}
