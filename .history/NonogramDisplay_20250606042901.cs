...
public abstract partial class NonogramDisplay : Container
{
	...
	public HintContainers HintContainers
	{
		get; init => field = InitContainers(value);
	...
	protected virtual (T columns, T rows) InitContainers<T>((T columns, T rows) hints) where T : BoxContainer
	{
		return (Init(hints.columns), Init(hints.rows));
		T Init(T value)
		{
			(value.SizeFlagsHorizontal, value.SizeFlagsVertical) = (SizeFlags.ExpandFill, SizeFlags.ExpandFill);
			value.SetAnchorsAndOffsetsPreset(
