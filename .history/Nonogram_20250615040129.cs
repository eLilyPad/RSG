...
public partial class Nonogram : Container
{
	...
	public sealed record PuzzleData : IData
	{
		...
		public bool HasSameState(Dictionary<Vector2I, bool> state)
		{
			return _tiles == state;
		}
	...
	public sealed partial class GameContainer(Nonogram nonogram) : NonogramDisplay
	{
		...
		public BoxContainer ToolBar { get; } = new HBoxContainer { Name = "Toolbar", SizeFlagsStretchRatio = 0.06f }
		.SizeFlags(horizontal: SizeFlags.ExpandFill, vertical: SizeFlags.ExpandFill);
		public BoxContainer ToolBarContainer { get; } = new VBoxContainer { Name = "ToolbarContainer" }
		.SizeFlags(horizontal: SizeFlags.ExpandFill, vertical: SizeFlags.ExpandFill)
		.AnchorsAndOffsetsPreset(preset: LayoutPreset.FullRect, resizeMode: LayoutPresetMode.KeepSize);

		private bool IsCorrect(IData data)
		{
			return Buttons.Values.All(button => button.Text is FillText);
		}
