...
public abstract partial class NonogramDisplay : Container
{
	...
	public sealed record Hints
	{
		...
		public (HintType, int position) SelectedHint
		{
			get; set
			{
				field = value;
		...
		private Dictionary<HintType, RichTextLabel[]> Labels { get; } = new([
			new (HintType.Row, []),
			new (HintType.Column, [])
		]);
		...
		public void PlaceLabels(int length)
		{
			...
			{
				RowsContainer.Add(Labels[HintType.Row][i] = Hint());
				ColumnsContainer.Add(Labels[HintType.Column][i] = Hint());
		...
		public void WriteToLabel(HintType type, int position, string text)
		{
