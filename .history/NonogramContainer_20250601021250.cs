...
public sealed partial class Nonogram : Container
{
	...
	) where T : IHavePenMode, IHaveColourPack
	{
		...
		Nonogram nonogram = new()
		{
			...
			Tiles = TilesContainer.Create(data, size, scale, margin),
			Hints = (new HintsContainer(length: 5), new HintsContainer(length: 5)),
		};
		return nonogram;
	}
	...
	where T : IHavePenMode, IHaveColourPack
	{
		var tilesContainer = new TilesContainer
		{
			...
			OnButtonPressed = button =>
			{
				...
				{
					Core.PenMode.Block when button.Text is EmptyText => BlockText,
					Core.PenMode.Block => EmptyText,
					//Core.PenMode.Block => button.Text switch { EmptyText => BlockText, _ => EmptyText },
					Core.PenMode.Fill => button.Text switch { EmptyText or BlockText => FillText, _ => EmptyText },
					_ => button.Text
				};
			}
		}.AnchorsAndOffsetsPreset(
