...
{
	public static TilesContainer Create<T>(T data, int size = 5) where T : IHavePenMode, IHaveColourPack
	{
		ColorRect background = new ColorRect
		{
			Name = "Background",
			Color = data.Colours.NonogramBackground,
			Size = Vector2I.One * GridLength * (GridScale + 5)
		}.AnchorsAndOffsetsPreset(
			preset: LayoutPreset.BottomRight,
			resizeMode: LayoutPresetMode.KeepSize,
			margin: GridMargin
		...
		TilesContainer tilesContainer = new()
		{
			...
			Columns = GridLength,
			Name = "Tiles",
			Size = Vector2I.One * GridLength * GridScale,
