...
public partial class Nonogram : Node, IHavePenMode
{
	...
	public DisplaySettings Settings
	{
		...
		{
			field = value;
			GameDisplay.Init(this);
			PainterDisplay.Init(this);
		}
	}
	...
	public (int Columns, Vector2 Size) TilesConfig => (Settings.Length, Settings.TileSize);
	public (Color Colour, Vector2 Size) BackgroundConfig => (Colours.NonogramBackground, Settings.TileSize);

	public override void _Ready()
	{
		Name = nameof(Nonogram);
		GameDisplay.Init(config: this);
		PainterDisplay.Init(config: this);
	...
	{
		public override void Init<Nonogram>(Nonogram config)
		{
			base.Init(config);
			(Background.Color, Background.Size) = nonogram.BackgroundConfig;
		...
		protected override T InitBackGround<T>(T value)
		{
			(value.Color, value.Size) = nonogram.BackgroundConfig;
			value.SetAnchorsAndOffsetsPreset(
				preset: LayoutPreset.FullRect,
				resizeMode: LayoutPresetMode.KeepSize,
				margin: nonogram.Settings.Margin
			);
			return base.InitBackGround(value);
		}
		protected override T InitTiles<T>(T value)
		{
			(value.Columns, value.Size) = nonogram.TilesConfig;
