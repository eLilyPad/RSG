using Godot;

namespace RSG;

using NonogramHint = UI.NonogramDisplay.TileHints;
using UI;

public partial class Nonogram : Node, IHavePenMode
{
	public enum Type { Game, Painter }
	public enum PenMode { Block, Fill }


	public PuzzleData CurrentData { get; private set; } = new();
	public PenMode CurrentPenMode { get; set; } = PenMode.Fill;
	public required ColourPack Colours { get; init => GameDisplay.Colours = PainterDisplay.Colours = field = value; }
	public DisplayConfig Config { get; set => GameDisplay.Config = PainterDisplay.Config = field = value; } = new();

	public GameContainer GameDisplay => field ??= new(nonogram: this) { Config = Config };
	public PaintContainer PainterDisplay => field ??= new(nonogram: this) { Config = Config };

	public override void _Ready()
	{
		Name = nameof(Nonogram);
		this.Add(
			//GameDisplay, 
			PainterDisplay
		);
		PainterDisplay.Hints.WriteToLabels(CurrentData);
	}

	private static void UpdateDisplay(Button button, PenMode mode)
	{
		button.Text = mode switch
		{
			PenMode.Block when button.Text is EmptyText or FillText => BlockText,
			PenMode.Block => EmptyText,
			PenMode.Fill when button.Text is EmptyText => FillText,
			PenMode.Fill when button.Text is FillText => EmptyText,
			_ => button.Text
		};
	}

	public sealed record PuzzleData : NonogramDisplay.IData
	{
		private readonly Dictionary<Vector2I, bool> _checkedTiles = [];

		public void Change(Vector2I position, bool clicked)
		{
			_checkedTiles[position] = clicked;
		}

		public Dictionary<NonogramHint.Position, List<int>> Hints()
		{
			Dictionary<NonogramHint.Position, List<int>> hints = [];
			foreach ((Vector2I position, bool check) in _checkedTiles)
			{
				NonogramHint.Position rowPos = new(NonogramHint.Side.Row, position.X),
					columnPos = new(NonogramHint.Side.Column, position.Y);

				if (!hints.TryGetValue(rowPos, out var row)) { hints[rowPos] = row = []; }
				if (!hints.TryGetValue(columnPos, out var column)) { hints[columnPos] = column = []; }

				if (check)
				{
					column.Add(1);
					row.Add(1);
				}
				else
				{
					column.Add(0);
					row.Add(0);
				}
			}

			return hints;
		}
	}

	public readonly record struct DisplayConfig(int Length = 5, int Scale = 40, int Margin = 150) : NonogramDisplay.IConfig
	{
		public readonly Vector2I Size => Vector2I.One * Length;
		public readonly Vector2I TilesSize => Size * Scale;
		public readonly Vector2I BackgroundSize => Size * (Scale + 5);
	}
	public sealed partial class PaintContainer(Nonogram nonogram) : NonogramDisplay
	{
		public override void OnTilePressed(Button button, Vector2I position)
		{
			UpdateDisplay(button, nonogram.CurrentPenMode);
			nonogram.CurrentData.Change(position, clicked: button.Text is FillText);
			Hints.WriteToLabels(nonogram.CurrentData);
		}
	}
	public sealed partial class GameContainer(Nonogram nonogram) : NonogramDisplay
	{
		public override void OnTilePressed(Button button, Vector2I position)
		{
			UpdateDisplay(button, nonogram.CurrentPenMode);
		}
	}
}