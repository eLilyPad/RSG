...
public abstract partial class NonogramDisplay : Container
{
	...
	public Hints GridHints => field ??= new(length: TilesLength);
	...
	public override void _Ready()
	{
		this.Add(
				Background,
				Main.Add(Spacer, GridHints.ColumnsContainer, GridHints.RowsContainer, Tiles)
			)
	...
	public sealed record Hints
	{
		...
		public static RichTextLabel Hint => new RichTextLabel { Text = "0", FitContent = true, CustomMinimumSize = TileSize }
			.SizeFlags(horizontal: SizeFlags.ExpandFill, vertical: SizeFlags.ExpandFill);

		private Dictionary<Position, RichTextLabel> Labels { get; } = [];

		public Hints(int length) { PlaceLabels(length); }
		...
		public void PlaceLabels(int length)
		{
			...
			{
				Position
					columnPos = new(Side: Side.Column, Index: i),
					rowPos = new(Side: Side.Column, Index: i);
				RowsContainer.Add(Labels[rowPos] = Hint);
				ColumnsContainer.Add(Labels[columnPos] = Hint);
			}
		}
