...
public abstract partial class NonogramDisplay : Container
{
	...
	public TilesContainer Tiles => field ??= new TilesContainer { Name = "Tiles", Columns = TilesLength }
	...
	public sealed partial class TilesContainer : GridContainer
	{
		...
		public void PlaceButtons(int length, Action<Button, Vector2I> pressed)
		{
			...
			{
				AddChild(Buttons[position] = CreateButton(position, pressed));
			}
		}

		private static Button CreateButton(Vector2I position, Action<Button, Vector2I> pressed)
		{
	...
	public sealed record TileHints
	{
		...
		{
			if (Labels.TryGetValue(position, out RichTextLabel? value))
			{
				value.Text = text;
			}
		}
