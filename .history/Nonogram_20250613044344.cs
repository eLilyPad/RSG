...
public partial class Nonogram : Node, IHavePenMode
{
	...
	public sealed record PuzzleData
	{
		...
		public Dictionary<NonogramDisplay.TileHints.Position, List<int>> Hints()
		{
			...
			{
				NonogramDisplay.TileHints.Position rowPos = new(NonogramDisplay.TileHints.Side.Row, position.Y);
				NonogramDisplay.TileHints.Position columnPos = new(NonogramDisplay.TileHints.Side.Column, position.X);
				hints.TryGetValue(key: columnPos, value: out List<int>? column);
				hints.TryGetValue(key: rowPos, value: out List<int>? row);
				hints[columnPos] ??= [];
				hints[rowPos] ??= [];
				if (check)
				{
