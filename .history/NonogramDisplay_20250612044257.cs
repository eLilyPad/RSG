...
public abstract partial class NonogramDisplay : Container
{
	...
	private int ChangeLength(int value)
	{
		...
		{
			GridHints.Rows.Add(GridHints.Labels[HintType.Row][i] = Hint());
			GridHints.Columns.Add(GridHints.Labels[HintType.Column][i] = Hint());
	...
	public sealed record Hints
	{
		...
		public void WriteHint(HintType type, int position, string text)
		{
			if (position > Labels[type].Length)
				Labels[type][position].Text = text;
		}
	}
}
