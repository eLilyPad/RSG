...
public abstract partial class NonogramDisplay : Container
{
	...
	{
		public readonly record struct Position(HintSide Side, int Index);
		...
		public Position SelectedHint
		{
			get; set
			{
				var labels = Labels[value.Side];
				Assert(
					condition: labels.Length <= value.Index || value.Index < 0,
					message: $"position is not within range, must be <= {labels.Length} and < 0, given position is {position}"
				);
				field = value;
			}
		}

		private Dictionary<HintSide, RichTextLabel[]> Labels { get; } = new([
			new (HintSide.Row, []),
			new (HintSide.Column, [])
		]);
		...
		public void PlaceLabels(int length)
		{
			...
			{
				RowsContainer.Add(Labels[HintSide.Row][i] = Hint());
				ColumnsContainer.Add(Labels[HintSide.Column][i] = Hint());
		...
		public void WriteToLabel(HintSide type, int position, string text)
		{
			var labels = Labels[type];
			Assert(
				condition: labels.Length <= position || position < 0,
				message: $"position is not within range, must be <= {labels.Length} and < 0, given position is {position}"
			);
