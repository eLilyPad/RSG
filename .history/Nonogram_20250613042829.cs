...
public partial class Nonogram : Node, IHavePenMode
{
	...
	public sealed record PuzzleData
	{
		...
		public Dictionary<NonogramDisplay.TileHints.Position, List<int>> Hints()
		{
			...
			{
				hints.TryGetValue(
					key: new(NonogramDisplay.TileHints.Side.Column, position.X),
					value: out List<int>? column
				);
				hints.TryGetValue(
					key: new(NonogramDisplay.TileHints.Side.Row, position.Y),
					value: out List<int>? row
				);
				column ??= [];
				row ??= [];
				if (check)
